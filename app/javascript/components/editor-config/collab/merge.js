...
function rebaseMergedTransform(
  ...
  const trNonConflict = recreateTransform(
    ...
    changes = ChangeSet.create(doc, {
      compare: (a, b) => false,
    }).addSteps(nonConflictingDoc, trNonConflict.mapping.maps, { user: 2 }),
      ...
      .map(([index, conflictStep]) => {
        ...
        if (mapped) {
          ...
          deleted = deleted.filter(
            (deleted) => deleted.data.user !== 1 || deleted.data.index !== index
          )
          return false
        }
      })
    ...
    conflictingSteps2 = conflictingSteps2
      .map(([index, conflictStep]) => {
        ...
        if (mapped) {
          ...
          deleted = deleted.filter(
            (deleted) => deleted.data.user !== 2 || deleted.data.index !== index
          )
          return false
        }
      })
